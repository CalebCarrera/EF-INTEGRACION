...
function Chat() {
  ...
  const [currentUser, setCurrentUser] = useState<"user1" | "user2" | null>(null); // Usuario actual (se asigna automáticamente)
  ...
  if (currentUser === null) {
    return (
      <div className="flex h-screen justify-center items-center">
        <div className="text-center">
          <h1 className="text-xl mb-4">Por favor, recargue la página</h1>
          <button
    ...
    ); // Espera a que el servidor asigne el rol
  }
      ...
      <div className="flex-1">
        ...
        <div className="bg-slate-300 h-screen overflow-y-auto p-4 pb-36">
          {messages.map((msg, index) => (
            <div key={index} className={`flex mb-4 ${msg.sender === "user1" ? "justify-start" : "justify-end"}`}>
              <div className={`flex max-w-96 rounded-lg p-3 gap-3 ${msg.sender === "user1" ? "bg-white text-gray-800" : "bg-indigo-400 text-white"}`}>
                <div className="w-9 h-9 rounded-full flex items-center justify-center">
                  <img
                    src={msg.sender === "user1" ? "https://placehold.co/200x/ffa8e4/ffffff.svg?text=ʕ•́ᴥ•̀ʔ&font=Lato" : "https://placehold.co/200x/b7a8ff/ffffff.svg?text=ʕ•́ᴥ•̀ʔ&font=Lato"}
                    alt="Avatar"
